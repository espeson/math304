MSE_Result_Digit_EachLam=[];
for lam = [10^-6,10^-3,10^0,10^3,10^6]
    for i=1:5
        MSE_Result_Digit_EachTestSet=[];
        TestData=LargeData(:,randi(100,[1 20]));
        TrainData=LargeData(:,randi(100,[1 80]));
        Parameter=LSR_xc166(TrainData,lam,9);
        syms x
        list=[];
        for a=0:size(Parameter)-1
            list=[list power(x,a)];
        end
        f(x)=Parameter'*list';
        %end of creating functions and start to plot 
        x_DataPoint_TestData=TestData(1,:);
        y_DataPoint_TestData=TestData(2,:);
        %end of plot and start to calculate the Training error
        MSE_Matrix_Temporary=[];
        for b=1:size(x_DataPoint_TestData,2)
            MSE=power(y_DataPoint_TestData(b)-f(x_DataPoint_TestData(b)),2);
            MSE_Matrix_Temporary=[MSE_Matrix_Temporary MSE];
        end
        MSE_Result_Digit_EachTestSet=[MSE_Result_Digit_EachTestSet sum(MSE_Matrix_Temporary)/size(x_DataPoint_TestData,2)];
    end
    MSE_Result_Digit_EachLam=[MSE_Result_Digit_EachLam sum(MSE_Result_Digit_EachTestSet)/5];
end
MSE_Result_Digit_EachLam=vpa(MSE_Result_Digit_EachLam);

MSE_Result_Digit=[];
Parameter=LSR_xc166(SmallData,10^-3,9);
%Start to create functions
syms x
list=[];
for a=0:size(Parameter)-1
    list=[list power(x,a)];
end
f(x)=Parameter'*list';
MSE_Matrix_Temporary=[];
for b=1:size(x_DataPoint_TestData,2)
    MSE=power(y_DataPoint_TestData(b)-f(x_DataPoint_TestData(b)),2);
    MSE_Matrix_Temporary=[MSE_Matrix_Temporary MSE];
end
MSE_Result_Digit=[MSE_Result_Digit sum(MSE_Matrix_Temporary)/size(x_DataPoint_TestData,2)];
MSE_Result_Digit=vpa(MSE_Result_Digit)





